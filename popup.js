...
var disable = document.getElementById('disablenow');
var enable = document.getElementById('enablenow');
...
function clearRules(arg) {
    ...
    if (arg == "contentSettings" || arg === undefined) {
        chromeContentSettings.images.clear({
            'scope': (incognito ? 'incognito_session_only' : 'regular')
        });
    }
...
function getLocalStoragePrefs() {
    ...
    if (!window.localStorage.img_on_off_prefs) {
        window.localStorage.img_on_off_prefs = JSON.stringify({
...
function openImgPanel() {
    return function(info, tab) {
        chrome.tabs.create({
            "url": "chrome://chrome/settings/contentExceptions#images",
            "selected": true
        });
    };
}
